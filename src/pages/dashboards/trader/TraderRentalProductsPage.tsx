...
// --- Main Page Component ---
export default function TraderRentalProductsPage({ onEdit = () => {} }: { onEdit?: (item: InventoryItem) => void }) {
  ...
  const items: InventoryItem[] = productsArray.map((p) => {
    ...
    return {
      ...
      stock: p.stock ?? 0,
      sku: p.sku || "",
      price: `$${p.rentalPrice ?? p.retailPrice ?? p.price ?? 0}`,
      priceNum: Number(p.rentalPrice ?? p.retailPrice ?? p.price) || 0,
