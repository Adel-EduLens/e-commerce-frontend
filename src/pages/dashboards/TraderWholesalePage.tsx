...
const earningsData = [
  { month: "Jan", value: 45000 }, { month: "Feb", value: 62000 },
  { month: "Mar", value: 48000 }, { month: "Apr", value: 71000 },
  { month: "May", value: 55000 }, { month: "Jun", value: 80000 },
  { month: "Jul", value: 67000 }, { month: "Aug", value: 87250 },
];
...
function EarningsChart() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="w-full" preserveAspectRatio="none">
      ...
      {pts.map((p) => <circle key={p.month} cx={p.x} cy={p.y} r={4} fill="white" stroke="#FFAE4C" strokeWidth="2" />)}
      {pts.map((p) => <text key={p.month + "l"} x={p.x} y={h - 6} textAnchor="middle" fontSize="9" fill="#6B7280">{p.month}</text>)}
    </svg>
  );
}
...
export default function TraderWholesalePage() {
  ...
  const { data: traderWholesales = [], isLoading, isError, error: errorMsg } = useTraderWholesales();
  ...
  const errorMessages = isError
    ? [(errorMsg as { response?: { data?: { message?: string } }; message?: string })?.response?.data?.message ?? "Failed to load wholesales"]
    : [];
  ...
  return (
    <>
      {showAddModal && <AddItemModal onClose={() => setShowAddModal(false)} lockedType="wholesale" />}
            ...
            <table className="min-w-full border-separate border-spacing-0">
              <thead>
                <tr className="bg-secondary">
                  <th className="px-4 py-3"><div className="h-5 w-5 rounded-md border border-primary bg-secondary" /></th>
                  {["Client", "Order", "Total Spent", "Last Order", "Status", "Actions"].map((col) => (
                ...
                {topClients.map((row, idx) => {
                  ...
                  return (
                    <tr key={idx} className={rowBg(idx)}>
                      <td className="px-4 py-3"><div className="h-5 w-5 rounded-md border border-stroke bg-white" /></td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-2xl px-2 py-1 text-xs font-medium font-['Montserrat'] ${pill.bg} ${pill.text}`}>{row.status}</span>
                      </td>
                      <td className="px-4 py-3">
