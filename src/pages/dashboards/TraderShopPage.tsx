  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        >
          <img className="h-5 w-5 brightness-0 invert" src={asset("ic_round-plus.svg")} alt="" />
          {t("addProduct") || "Add Product"}
        </button>
      </div>

      <div className="mt-4">
        {activeTab === "products" && <TraderProductsPage onEdit={setEditItem} />}
        {activeTab === "retail" && <TraderRetailProductsPage onEdit={setEditItem} />}
        {activeTab === "wholesale" && <TraderWholeSaleProductsTable onEdit={setEditItem} />}
        {activeTab === "blank" && <TraderBlankProductsPage onEdit={setEditItem} />}
      </div>
    </div>
  );
}