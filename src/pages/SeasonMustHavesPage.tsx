...
function ArrowCircle({ size = 32 }: { size?: 32 | 40 | 48 }) {
  const dimensions =
    size === 48 ? "h-12 w-12" : size === 40 ? "h-10 w-10" : "h-8 w-8";
  const iconPosition =
    size === 48
      ? "left-[18px] top-[12px] h-6 w-3"
      : size === 40
        ? "left-[14px] top-[8px] h-6 w-3"
        : "left-[10px] top-[4px] h-6 w-3";

  return (
    <div className={`relative ${dimensions} overflow-hidden rounded-full bg-white`}>
      <AssetImage
        file="weui_arrow-filled-3.svg"
        className={`absolute ${iconPosition}`}
      />
    </div>
  );
}
...
function Footer() {
  ...
  return (
    <div className="absolute left-0 top-[1784px] h-96 w-[1440px] overflow-hidden border-t border-[#E0E0E0]">
