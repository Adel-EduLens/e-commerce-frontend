  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        items: checkoutItems.map((item) => ({
          productId: item.productId,
          categoryId: item.categoryId || (item.category?.id ? String(item.category.id) : null),
          title: item.title,
          unitPrice: item.unitPrice,
          quantity: item.quantity,
          size: item.size || null,
          ...
          productType: item.productType === "RETAIL" ? "RENTAL" : item.productType === "STANDARD" ? "SHOP" : (item.productType || "SHOP"),
          recipientName: item.recipientName || null,
          recipientEmail: item.recipientEmail || null,
          giftMessage: item.giftMessage || null,
        })),
