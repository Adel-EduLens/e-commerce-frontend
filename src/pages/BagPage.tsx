...
function ArrowCircle({ size = 48 }: { size?: 40 | 48 }) {
  return (
    <div
      className={`relative ${size === 48 ? "h-12 w-12" : "h-10 w-10"} overflow-hidden rounded-full bg-white`}
    >
      <AssetImage
        file="weui_arrow-filled-3.svg"
        className={`absolute ${size === 48 ? "left-[18px] top-[12px] h-6 w-3" : "left-[14px] top-[8px] h-6 w-3"}`}
      />
    </div>
  );
}
...
function Footer() {
  ...
  return (
    <div className="absolute left-0 top-[1507px] h-96 w-[1440px] overflow-hidden border-t border-[#E0E0E0]">
